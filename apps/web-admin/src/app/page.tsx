...
function DashboardBody({ session }: { session: AdminSession }) {
  ...
  const can = (permission: AdminPermission) => hasPermission(session.role, permission);
  ...
  const visibleKpis = [
    can('VIEW_TICKETS') && <KpiTile key="tickets" icon={Ticket} label="Daily tickets sold" value={kpis.ticketsToday.toLocaleString('en-NG')} delta={{ value: '+12% vs yest.', positive: true }} />,
    can('VIEW_FINANCE') && <KpiTile key="revenue" icon={Receipt} label="Gross revenue" value={formatNaira(kpis.grossTodayNgn)} hint="Settled + pending" />,
    can('VIEW_AGENTS') && <KpiTile key="agents" icon={UserCog} label="Active agents" value={String(kpis.activeAgents)} hint="ACTIVE status" />,
    can('VIEW_REPORTS') && <KpiTile key="compliance" icon={ShieldCheck} label="Remit compliance" value={`${Math.round(kpis.remittanceCompliance * 100)}%`} tone={kpis.remittanceCompliance < 0.85 ? 'warning' : 'success'} hint="Rolling 30-day" />,
    can('VIEW_FINANCE') && <KpiTile key="fund" icon={Gauge} label="Jackpot fund" value={formatNaira(kpis.jackpot.balanceNgn)} tone={jackpotTone} hint={`Traffic light · ${kpis.jackpot.state}`} />,
    can('VIEW_DRAWS') && <KpiTile key="draw" icon={Clock} label="Next draw" value={countdown || '—'} hint={kpis.nextDrawPrize} />,
  ].filter(Boolean);

  return (
    <>
      <PageHeader
        eyebrow={`${roleLabel(session.role)} dashboard`}
        title="Role-based view"
        description={roleDescription(session.role)}
        rightSlot={
