...
import { getPrizePayoutSummary, type AgentPrizePayoutSummary } from '@/lib/prize-payout-ledger';
...
function DashboardBody({ agent }: { agent: import('@surewina/types').AgentMe }) {
  ...
  const [payoutSummary, setPayoutSummary] = useState<AgentPrizePayoutSummary>({
    count: 0,
    totalPaidNgn: 0,
    latestPaidAt: null,
  });
  ...
  useEffect(() => {
    ...
    agentMock.listSales(period).then((salesData) => {
      if (!active) return;
      setSales(salesData);
      setPayoutSummary(getPrizePayoutSummary());
      setLoading(false);
    });
      ...
      <section className="rounded-3xl bg-navy-800 p-5 text-white shadow-[0_24px_70px_rgba(14,42,71,0.16)] sm:p-6">
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,1fr)_280px] lg:items-center">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-white/75">Sell tickets, track commission, record prize payouts, and settle the correct net remittance before cutoff.</p>
          </div>
      ...
      <section className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <aside className="space-y-4">
          <Card variant="default" className="rounded-3xl border-slate-200 bg-white p-5 shadow-sm">
            <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-sm bg-amber-50 text-amber-700"><Banknote className="h-5 w-5" /></div>
...
function FinanceRow({ label, value, strong = false }: { label: string; value: string; strong?: boolean }) {
  return <div className="flex items-center justify-between gap-3 rounded-2xl border border-slate-100 bg-navy-50 px-3 py-2"><p className="text-xs font-bold text-slate-500">{label}</p><p className={strong ? 'font-display text-base font-black text-navy-950' : 'font-display text-sm font-black text-navy-950'}>{value}</p></div>;
}
