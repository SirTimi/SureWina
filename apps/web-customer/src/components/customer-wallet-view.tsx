...
export function CustomerWalletView({
  ...
  const loadWalletData = useCallback(async () => {
    ...
    try {
      const [walletResult, fundingResult, historyResult] = await Promise.all([
        api.wallet.getMine(),
        api.wallet.fundingHistory(1, 10),
        api.wallet.history(1, 10),
      ]);
  ...
  const verifyReturnedFunding = useCallback(async () => {
    ...
    try {
      const funding = await api.wallet.fundingStatus(
        fundingReference,
      ...
      if (funding.status === 'CREDITED') {
        ...
        const fundingHistory = await api.wallet.fundingHistory(1, 10);
  ...
  const handleTopUp = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const funding = await api.wallet.initiateFunding({
        gateway,
  ...
  if (loadError || !wallet) {
    return (
      <Card
        variant="default"
        className="rounded-3xl border-red-100 bg-white p-8 text-center shadow-sm"
      >
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-6">
          <Card
            variant="default"
            className="overflow-hidden rounded-3xl border-navy-100 bg-navy-800 text-white shadow-[0_24px_70px_rgba(14,42,71,0.18)]"
          >
              ...
              <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-sm text-white/70">
                    Ready to use from your SureWina wallet.
                  </p>
                </div>
          ...
          <Card
            variant="default"
            className="rounded-3xl border-slate-200 bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.04)]"
          >
            ...
            {transactions.length === 0 ? (
              <EmptyState
                ...
                body="Your top-ups and wallet transactions will appear here."
              />
            ) : (
          ...
          >
            ...
            {fundings.length === 0 ? (
              <EmptyState
                ...
                body="Once you fund your wallet, the payment and verification status will appear here."
              />
            ) : (
              <div className="divide-y divide-slate-100">
        ...
        <aside className="space-y-4 lg:sticky lg:top-28 lg:self-start">
          ...
          >
            ...
            <p className="mt-2 text-sm leading-relaxed text-slate-500">
              Add money securely using Monnify or Flutterwave. Your balance is credited only after SureWina verifies the payment.
            </p>
            ...
            <form className="mt-6 space-y-5" onSubmit={handleTopUp}>
              ...
              <Button
                ...
                variant="accent"
                fullWidth
                disabled={startingFunding || wallet.status !== 'ACTIVE'}
            ...
            <div className="mt-5 flex items-start gap-2 border-t border-slate-100 pt-4 text-xs leading-relaxed text-slate-500">
              ...
              SureWina does not credit a wallet from browser-returned payment status. The API verifies the transaction with the provider first.
            </div>
          </Card>

          <Card
            variant="default"
            className="rounded-3xl border-navy-100 bg-amber-50 p-5 shadow-[0_18px_50px_rgba(15,23,42,0.03)]"
          >
            <p className="text-sm font-black text-navy-950">
              Ready to play?
            </p>
            <p className="mt-2 text-sm leading-relaxed text-slate-600">
              Browse the current draws while your wallet activity stays available here.
            </p>
    ...
    >
      <div className="flex items-center justify-between gap-3">
        ...
        <span
          className={
            selected
              ? 'flex h-4 w-4 items-center justify-center rounded-full border-4 border-navy-700 bg-white'
...
function ReturnedFundingStatus({
  ...
  if (funding.status === 'CREDITED') {
    return (
      <StatusNotice
        ...
        body={`${formatNaira(funding.amountNgn)} was verified and added to your wallet.`}
      />
    );
  }
  ...
  return (
    <StatusNotice
      ...
      body="The provider has not completed verification yet. You can refresh the status without making another payment."
      action={
        <button
          type="button"
          onClick={onRefresh}
...
function StatusNotice({
  ...
  const styles = {
    ...
    info: {
      wrapper: 'border-navy-100 bg-navy-50',
      icon: 'text-navy-700',
      title: 'text-navy-950',
    },
  }[tone];
...
function FundingStatusPill({
  status,
}: {
  status: WalletFundingView['status'];
}) {
  const classes =
    status === 'CREDITED'
